...
Quintus.TMX = function(Q) {
 ...
 Q._tmxProcessImageLayer = function(stage,gidMap,tileProperties,layer) {
   console.log("_tmxProcessImageLayer");
...
//_______________________________________________ BELOW ADDED BY VILVA FOR stageJSON
  
  function jsonParseProperties(elem) {
   var propElems = elem.properties,
       props = {};

   if(propElems) {
	   for(var i = 0; i < propElems.length; i++) {
		 var propElem = propElems[i];
		 props[propElem.name] = propElem.value;
	   }
   }
   return props;
 }
  
  Q._jsonExtractAssetName = function(source) {
   var sourceParts = source.split("/");
   // only return the last part of the asset string
   return sourceParts[sourceParts.length - 1];
 };
  
  Q._jsonLoadTilesets = function(tilesets, tileProperties) {
   var gidMap = [];
   ...
   for(var t = 0; t < tilesets.length; t++) {
     var tileset = tilesets[t],
         ...
         assetName = Q._jsonExtractAssetName(tileset.image),
         tilesetTileProps = {},
         tilesetProps = { tileW: tileset.tilewidth,
                          tileH: tileset.tileheight,
                          spacingX: tileset.spacing,
                          spacingY: tileset.spacing
                        };
	 ...
	 if(tiles) {
		 for(var i = 0;i < tiles.length;i++) {
		   ...
		   var properties = jsonParseProperties(tile);
  ...
  Q._jsonProcessTileLayer = function(stage,gidMap,tileProperties,layer) {
   console.log("_jsonProcessTileLayer");
 ...
 Q._jsonProcessObjectLayer = function(stage,gidMap,tileProperties,layer) {
   console.log("_jsonProcessObjectLayer");
   ...
   for(var i=0;i < objects.length;i++) {
     var obj = objects[i],
         ...
         y = obj.y,
         properties = tileProperties[gid],
         overrideProperties = jsonParseProperties(obj);
  ...
  Q._jsonProcessors = { 'objectgroup': Q._jsonProcessObjectLayer,
                      'layer': Q._jsonProcessTileLayer };
  
  Q.stageJSON = function(data, stage) {
    var tileProperties = {};

    // Load Tilesets
    var tilesets = data.tilesets;
    var gidMap = Q._jsonLoadTilesets(tilesets, tileProperties);

    // Go through each of the layers
    Q._each(data.layers, function(layer) {
      var layerType = layer.type;
      if(Q._jsonProcessors[layerType]) {
        Q._jsonProcessors[layerType](stage, gidMap, tileProperties, layer);
      }
    });
  };
